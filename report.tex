...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    arr/.style={-{Stealth[length=6pt]}, thick, color=kiitblue!60},
  ]
    %% Row 1: Pipeline stages (left to right)
    \node[box]   at (0,0)    (api)    {OpenWeatherMap\\API};
    \node[box]   at (3.8,0)  (ingest) {Ingestion\\Layer};
    \node[box]   at (7.6,0)  (proc)   {Processing\\Layer};
    \node[box]   at (11.4,0) (dq)     {Data Quality\\Gate};
    %% Row 2: Storage / presentation (right to left mirrors row 1)
    \node[store] at (11.4,-2) (sqlite) {SQLite\\Star Schema};
    \node[store] at (7.6,-2)  (parq)   {Parquet\\Data Lake};
    \node[box]   at (3.8,-2)  (dash)   {Streamlit\\Dashboard};
    %% Scheduler label
    \node[font=\footnotesize\itshape, text=darkgray] at (1.9,-1) {APScheduler};
    %% Arrows row 1
    \draw[arr] (api)    -- (ingest);
    ...
    %% Scheduler loop arrow
    \draw[arr, dashed] (0,-1) -- (ingest.south west);
  \end{tikzpicture}
  \end{tcolorbox}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[dim]  at (-4.2, 0) (dd)  {dim\_date};
    \node[dim]  at ( 4.2, 0) (dco) {dim\_condition};
