...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    store/.style={draw=accentteal, fill=accentteal!10, rounded corners=4pt,
                  minimum width=3.0cm, minimum height=1.0cm,
                  font=\small\bfseries, text=accentteal, align=center},
    sched/.style={draw=darkgray!70, fill=lightgray, rounded corners=4pt,
                  minimum width=3.0cm, minimum height=1.0cm,
                  font=\small\bfseries, text=darkgray, align=center},
    arr/.style={-{Stealth[length=6pt]}, thick, color=kiitblue!70},
    darr/.style={-{Stealth[length=6pt]}, thick, color=darkgray, dashed},
  ]
    %% Row 1: Pipeline stages (left to right)
    \node[box]   at (0,0)    (api)    {OpenWeatherMap\\API};
    \node[box]   at (4.2,0)  (ingest) {Ingestion\\Layer};
    \node[box]   at (8.4,0)  (proc)   {Processing\\Layer};
    \node[box]   at (12.6,0) (dq)     {Data Quality\\Gate};
    %% Row 2: Storage / presentation (right to left mirrors row 1)
    \node[store] at (12.6,-2) (sqlite) {SQLite\\Star Schema};
    \node[store] at (8.4,-2)  (parq)   {Parquet\\Data Lake};
    \node[box]   at (4.2,-2)  (dash)   {Streamlit\\Dashboard};
    %% APScheduler as a proper labeled box (bottom-left)
    \node[sched] at (0,-2)    (sched)  {APScheduler};
    %% Row 1 arrows
    \draw[arr]  (api)    -- (ingest);
    \draw[arr]  (ingest) -- (proc);
    \draw[arr]  (proc)   -- (dq);
    %% Clean U-turn: straight down then chain left
    \draw[arr]  (dq)     -- (sqlite);
    \draw[arr]  (sqlite) -- (parq);
    \draw[arr]  (parq)   -- (dash);
    %% APScheduler triggers the API fetch (dashed)
    \draw[darr] (sched)  -- (api);
  \end{tikzpicture}
  \end{tcolorbox}
...
\begin{figure}[H]
  ...
  \centering
  \begin{tikzpicture}[
    dim/.style={draw=kiitblue, fill=kiitblue!15, rounded corners=4pt,
                minimum width=3.0cm, minimum height=1.0cm,
                font=\small\bfseries, text=kiitblue, align=center},
    fact/.style={draw=kiitgold!80!black, fill=kiitgold!20, rounded corners=4pt,
    ...
    lbl/.style={font=\tiny\itshape, text=darkgray},
  ]
    %% Central fact table
    \node[fact] at (0,0)    (fw)  {fact\_weather\\(hourly grain)};
    %% Dimension tables at cardinal positions
    \node[dim]  at (0, 2.8)  (dc)  {dim\_city};
    \node[dim]  at (-4.8, 0) (dd)  {dim\_date};
    \node[dim]  at ( 4.8, 0) (dco) {dim\_condition};
    %% Arrows with FK labels
    \draw[arr] (dc)  -- (fw) node[lbl, right=2pt, midway] {city\_id};
    \draw[arr] (dd)  -- (fw) node[lbl, above=2pt, midway] {date\_id};
    \draw[arr] (dco) -- (fw) node[lbl, above=2pt, midway] {condition\_id};
  \end{tikzpicture}
  \end{tcolorbox}
